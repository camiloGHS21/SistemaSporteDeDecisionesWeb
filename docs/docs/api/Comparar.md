--- 
sidebar_label: 'Comparar'
--- 

# Componente Comparar

**Ruta del archivo:** `src/pages/Comparar.jsx`

## Descripción

Página para comparar el rendimiento de las políticas digitales entre países. Permite a los usuarios seleccionar países e indicadores, ver los datos en un gráfico y chatear con un asistente de IA para analizar los resultados.
...
- **comparisonData**: `object | null` - Almacena los datos de la comparación para el gráfico. Por defecto es `null`.
- **isChatOpen**: `boolean` - Controla si la ventana de chat del asistente de IA está abierta. Por defecto es `false`.
- **messages**: `Array<object>` - Un array de mensajes para el chat del asistente de IA.
- **indicators**: `Array<object>` - Un array de los indicadores disponibles para la comparación.
- **isGenerating**: `boolean` - Indica si se está generando una comparación. Por defecto es `false`.
...
- `{JSX.Element}`: La página de comparación renderizada.
...
const Comparar = () => {
  ...
  const [messages, setMessages] = useState([
    {
      sender: 'bot',
      text: 'Â¡Hola! Soy tu asistente de IA. Pregóntame\nsobre los datos en el gráfico.',
    },
  ]);
